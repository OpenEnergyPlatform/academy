...
def json_extraction(sql_input, json_output="old_json.json"):
    """Extracts the json string from an existing COMMENT ON TABLE query file
    to an output file.
...
def json_conversion(
    tablename, username, user_email, json_old_input, json_new_output="json_new.json"
):
    ...
    # In the following a new structure is set. The yet implemented version is
    # for metadata v1.2 to v1.3.
    ...
    d["contributors"][index]["date"] = "{}-{}-{}".format(now.year, now.month, now.day)
    d["contributors"][index][
        "comment"
    ] = "Update metadata to v1.3 using metadata conversion tool"
    ...
    for i in range(len(json_old["resources"])):
        ...
        for j in range(len(json_old["resources"])):
            ...
            for k in range(len(json_old["resources"][i]["fields"])):
                ...
                d["resources"][i]["schema"][j]["fields"][k]["name"] = json_old[
                    "resources"
                ][i]["fields"][k]["name"]
                d["resources"][i]["schema"][j]["fields"][k]["description"] = json_old[
                    "resources"
                ][i]["fields"][k]["description"]
                d["resources"][i]["schema"][j]["fields"][k]["type"] = ""
                d["resources"][i]["schema"][j]["fields"][k]["unit"] = json_old[
                    "resources"
                ][i]["fields"][k]["unit"]
    ...
    d_comment["metadata"] = (
        "Metadata documentation and explanation "
...
class MyJSONEncoder(json.JSONEncoder):
    """This enconder sets up a structured oder of the json string when transforming it
    from a python dict"""
                ...
                for item in o:
                    ...
                    if isinstance(item, dict):
                        ...
                        raise AssertionError(
                            "Only dicts in lists are properly structured. "
                            "Please redefine it in the encode function."
                        )
...
def metadata_conversion(old_sql, new_sql, user, user_email, only_endfiles=True):
    """Conversion of an existing metadata file to a newer version by using
    the above functions.
    ...
    json_new_string = json_conversion(
        tablename, user, user_email, json_old_string, json_new_output="json_new.json"
    )
